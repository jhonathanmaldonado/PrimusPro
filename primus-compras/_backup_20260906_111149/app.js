...
// Limpeza - Saídas (Fase 2)
let consumoLimpeza = [];
let limpezaSolicitantes = [];
let currentSubTabLimpeza = 'painel';    // painel | saidas
let saidasMes = 'todos';                // 'todos' | 'YYYY-MM'
let saidasPessoa = 'todos';             // 'todos' | nome
let searchLimpezaCatalogo = '';
...
function iniciarListeners() {
  ...
  unsubsRefs.push(observarItens((its) => {
    itens = its;
    renderTudo();
    if ($('modal-limpeza-config').classList.contains('show')) renderConfigItensLimpeza();
  ...
  unsubsRefs.push(observarConsumoLimpeza((c) => {
    consumoLimpeza = c;
    if (currentTab === 'limpeza' && currentSubTabLimpeza === 'saidas') renderSaidas();
    if ($('modal-limpeza-config').classList.contains('show')) renderConfigSolicitantes();
  }));

  unsubsRefs.push(observarSolicitantesLimpeza((s) => {
    limpezaSolicitantes = s;
    if (currentTab === 'limpeza' && currentSubTabLimpeza === 'saidas') renderSaidas();
    if ($('modal-limpeza-config').classList.contains('show')) renderConfigSolicitantes();
  }));
}
...
function switchTab(tab) {
  ...
  if (tab === 'limpeza') switchSubTabLimpeza(currentSubTabLimpeza || 'painel');
...
// ============================================================================
// LIMPEZA — Saídas do depósito (Fase 2)
// ============================================================================

function switchSubTabLimpeza(sub) {
  currentSubTabLimpeza = sub;
  document.querySelectorAll('[data-limpeza-subtab]').forEach(b => {
    b.classList.toggle('active', b.dataset.limpezaSubtab === sub);
  });
  $('limpeza-subtab-painel').style.display = sub === 'painel' ? 'block' : 'none';
  $('limpeza-subtab-saidas').style.display = sub === 'saidas' ? 'block' : 'none';

  if (sub === 'painel') renderLimpeza();
  if (sub === 'saidas') renderSaidas();
}

// Itens do catálogo marcados como controlados no depósito
function itensControlados() {
  return itens
    .filter(i => i.controleLimpeza)
...
function saidasMesesDisponiveis() {
  const meses = new Set();
  for (const s of consumoLimpeza) {
    if (s.data && s.data.length >= 7) meses.add(s.data.substring(0, 7));
  }
  return Array.from(meses).sort().reverse();
}

function saidasDoPeriodo() {
  let lista = consumoLimpeza.slice();
  if (saidasMes !== 'todos') {
    lista = lista.filter(s => s.data && s.data.startsWith(saidasMes));
  }
  if (saidasPessoa !== 'todos') {
    lista = lista.filter(s => (s.solicitante || 'Não informado') === saidasPessoa);
  }
  return lista.sort((a, b) => (b.data || '').localeCompare(a.data || ''));
}

// ============================================================================
// LIMPEZA — Modal de registrar saída
// ============================================================================

function abrirModalSaida() {
  const controlados = itensControlados();

  if (!controlados.length) {
    showToast('⚠ Nenhum item controlado ainda. Abra ⚙️ Configurar primeiro.', 'error');
    abrirModalLimpezaConfig();
    return;
  }
  ...
  const ativos = limpezaSolicitantes.filter(s => s.ativo !== false);
  selSol.innerHTML = ativos.length
    ? ativos.map(s => `<option value="${s.id}">${escHtml(s.nome)}</option>`).join('')
    : '<option value="">— cadastre em ⚙️ Configurar —</option>';

  $('saida-qtd').value = '';
  $('saida-obs').value = '';
  const hoje = new Date();
  $('saida-data').value = `${hoje.getFullYear()}-${String(hoje.getMonth() + 1).padStart(2, '0')}-${String(hoje.getDate()).padStart(2, '0')}`;
  $('saida-error').classList.remove('show');

  atualizarPreviewSaida();
  $('modal-saida').classList.add('show');
  setTimeout(() => $('saida-qtd').focus(), 80);
}

function fecharModalSaida() {
  $('modal-saida').classList.remove('show');
}
...
async function salvarSaida() {
  const err = $('saida-error');
  err.classList.remove('show');

  const item = itens.find(i => i.id === $('saida-item').value);
  const qtd = parseFloat($('saida-qtd').value) || 0;
  const data = $('saida-data').value;
  const solId = $('saida-solicitante').value;
  const sol = limpezaSolicitantes.find(s => s.id === solId);

  if (!item) { err.textContent = 'Escolha um item.'; err.classList.add('show'); return; }
  if (qtd <= 0) { err.textContent = 'Quantidade precisa ser maior que zero.'; err.classList.add('show'); return; }
  if (!data) { err.textContent = 'Informe a data.'; err.classList.add('show'); return; }
  if (!sol) { err.textContent = 'Escolha quem pediu (cadastre em ⚙️ Configurar).'; err.classList.add('show'); return; }
...
// ============================================================================
// LIMPEZA — Modal de configuração (itens controlados + solicitantes)
// ============================================================================

function abrirModalLimpezaConfig() {
  searchLimpezaCatalogo = '';
  $('search-limpeza-catalogo').value = '';
  $('novo-solicitante-nome').value = '';
  $('limpeza-config-error').classList.remove('show');
  renderConfigItensLimpeza();
  renderConfigSolicitantes();
  $('modal-limpeza-config').classList.add('show');
}

function fecharModalLimpezaConfig() {
  $('modal-limpeza-config').classList.remove('show');
}

function renderConfigItensLimpeza() {
  const el = $('limpeza-config-itens');
  const catId = limpezaResolverCategoria();
  const termo = searchLimpezaCatalogo.toLowerCase();

  // Mostra a categoria de limpeza por padrão; a busca varre o catálogo inteiro
  let lista = termo
    ? itens.filter(i => (i.nome || '').toLowerCase().includes(termo))
    : itens.filter(i => i.categoriaId === catId);

  lista = lista.sort((a, b) => {
    const ca = a.controleLimpeza ? 0 : 1;
    const cb = b.controleLimpeza ? 0 : 1;
    if (ca !== cb) return ca - cb;
    return (a.nome || '').localeCompare(b.nome || '', 'pt-BR');
  });

  if (!lista.length) {
    el.innerHTML = '<div class="empty-msg">Nenhum item encontrado</div>';
    return;
  }

  let html = '';
  for (const i of lista.slice(0, 60)) {
    const custo = custoUnitarioItem(i);
    const avisoPreco = custo > 0
  ...
  if (lista.length > 60) {
    html += `<div style="padding:8px 12px;font-size:11px;color:var(--muted);text-align:center">mostrando 60 de ${lista.length} — use a busca</div>`;
  }
...
async function toggleControleLimpeza(itemId, controlado) {
  try {
    await setItemControleLimpeza(itemId, controlado);
  } catch (e) {
    showToast('⚠ Erro: ' + e.message, 'error');
    renderConfigItensLimpeza();
  }
...
function renderConfigSolicitantes() {
  const el = $('limpeza-config-solicitantes');
  if (!limpezaSolicitantes.length) {
    el.innerHTML = '<div class="empty-msg">Ninguém cadastrado ainda</div>';
    return;
  }

  let html = '';
  for (const s of limpezaSolicitantes) {
    const usos = consumoLimpeza.filter(c => c.solicitanteId === s.id).length;
    html += `<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;border-bottom:1px solid var(--line)">`;
    html += `<span style="flex:1;font-size:13px">${escHtml(s.nome)}</span>`;
    html += `<span style="font-size:11px;color:var(--muted)">${usos} ${usos === 1 ? 'saída' : 'saídas'}</span>`;
    html += `<button class="vinculo-btn-sm danger" data-action="del-solicitante" data-sol-id="${s.id}">Remover</button>`;
    html += `</div>`;
  }
  el.innerHTML = html;
}

async function adicionarSolicitante() {
  const err = $('limpeza-config-error');
  err.classList.remove('show');
  const nome = $('novo-solicitante-nome').value.trim();
  if (!nome) { err.textContent = 'Digite um nome.'; err.classList.add('show'); return; }

  try {
    await criarSolicitanteLimpeza(nome);
    $('novo-solicitante-nome').value = '';
...
async function removerSolicitante(id) {
  const s = limpezaSolicitantes.find(x => x.id === id);
  if (!s) return;
  const usos = consumoLimpeza.filter(c => c.solicitanteId === s.id).length;
  let msg = `Remover ${s.nome} da lista?`;
  if (usos > 0) {
    msg += `\n\nAs ${usos} saída(s) já registradas continuam no histórico com o nome dela — nada é apagado.`;
  }
  if (!confirm(msg)) return;

  try {
    await deletarSolicitanteLimpeza(id);
...
// ============================================================================
// LIMPEZA — Render da sub-aba Saídas
// ============================================================================

function renderSaidas() {
  // Filtro de mês
  const meses = saidasMesesDisponiveis();
  const selMes = $('saidas-filtro-mes');
  const opcoesMes = ['<option value="todos">Todos os meses</option>']
    .concat(meses.map(m => `<option value="${m}">${nomeMes(m)}</option>`));
  selMes.innerHTML = opcoesMes.join('');
  if (saidasMes !== 'todos' && !meses.includes(saidasMes)) saidasMes = meses[0] || 'todos';
  selMes.value = saidasMes;

  // Filtro de pessoa
  const pessoas = Array.from(new Set(consumoLimpeza.map(s => s.solicitante || 'Não informado'))).sort();
  const selPessoa = $('saidas-filtro-pessoa');
  selPessoa.innerHTML = ['<option value="todos">Todas</option>']
    .concat(pessoas.map(p => `<option value="${escHtml(p)}">${escHtml(p)}</option>`)).join('');
  if (saidasPessoa !== 'todos' && !pessoas.includes(saidasPessoa)) saidasPessoa = 'todos';
  selPessoa.value = saidasPessoa;

  const lista = saidasDoPeriodo();
  const custoTotal = lista.reduce((s, x) => s + (parseFloat(x.custoTotal) || 0), 0);
  const semCusto = lista.filter(x => !(parseFloat(x.custoTotal) > 0)).length;
  const pessoasPeriodo = new Set(lista.map(s => s.solicitante || 'Não informado'));

  $('saidas-total-registros').textContent = lista.length;
  $('saidas-total-custo').textContent = fmtMoeda(custoTotal);
  $('saidas-total-pessoas').textContent = lista.length ? pessoasPeriodo.size : 0;
  $('saidas-custo-meta').textContent = semCusto > 0
    ? `${semCusto} sem preço conhecido`
    : 'todas com custo';
  $('saidas-resumo-texto').textContent = saidasMes === 'todos'
    ? `${consumoLimpeza.length} registros no total`
    : nomeMes(saidasMes);

  // Avisos
  const avisos = [];
  if (semCusto > 0) {
    avisos.push(`<strong>${semCusto}</strong> saída(s) entraram com custo R$ 0,00 porque o item ainda não tem preço de compra registrado. O custo total está subestimado.`);
  }
  if (consumoLimpeza.length >= 800) {
    avisos.push('O painel carrega as <strong>800 saídas mais recentes</strong>. Registros mais antigos que isso não entram nos totais.');
  }
  const aviso = $('saidas-aviso');
  if (avisos.length) {
    aviso.innerHTML = 'ℹ️ ' + avisos.join('<br><br>ℹ️ ');
    aviso.style.display = 'block';
  } else {
  ...
  renderSaidasPorPessoa(lista, custoTotal);
  renderSaidasPorItem(lista, custoTotal);
  renderSaidasLista(lista);
}

function renderSaidasPorPessoa(lista, custoTotal) {
  const el = $('saidas-tabela-pessoas');
  if (!lista.length) {
    el.innerHTML = '<div class="empty-msg">Nenhuma saída no período</div>';
    return;
  }

  const agg = {};
  for (const s of lista) {
    const nome = s.solicitante || 'Não informado';
    if (!agg[nome]) agg[nome] = { nome, custo: 0, registros: 0, itens: new Set() };
    agg[nome].custo += parseFloat(s.custoTotal) || 0;
    agg[nome].registros++;
    agg[nome].itens.add(s.itemNome || '?');
  }

  const linhas = Object.values(agg).sort((a, b) => b.custo - a.custo);

  let html = '<table class="cmv-tabela">';
  html += '<thead><tr><th>Pessoa</th><th class="num">Retiradas</th><th class="num">Itens diferentes</th><th class="num">Custo</th><th class="num">% do total</th></tr></thead><tbody>';
  for (const p of linhas) {
    const share = custoTotal > 0 ? (p.custo / custoTotal) * 100 : 0;
    html += '<tr>';
    html += `<td><strong>${escHtml(p.nome)}</strong></td>`;
    html += `<td class="num">${p.registros}</td>`;
    html += `<td class="num">${p.itens.size}</td>`;
    html += `<td class="num">${fmtMoeda(p.custo)}</td>`;
    html += `<td class="num">${share.toFixed(1)}%</td>`;
    html += '</tr>';
  }
...
function renderSaidasPorItem(lista, custoTotal) {
  const el = $('saidas-tabela-itens');
  if (!lista.length) {
    el.innerHTML = '<div class="empty-msg">Nenhuma saída no período</div>';
    return;
  }
  ...
  let html = '<table class="cmv-tabela">';
  html += '<thead><tr><th>Item</th><th class="num">Qtd</th><th class="num">Saídas</th><th class="num">Custo</th><th class="num">% do total</th></tr></thead><tbody>';
  for (const i of linhas) {
    const share = custoTotal > 0 ? (i.custo / custoTotal) * 100 : 0;
...
function renderSaidasLista(lista) {
  const el = $('saidas-lista');
  if (!lista.length) {
    el.innerHTML = '<div class="empty-msg">Nenhuma saída registrada no período</div>';
    return;
  }

  const podeExcluir = userCtx?.role === 'dono' || userCtx?.role === 'membro';

  let html = '';
  let dataAtual = null;
  for (const s of lista.slice(0, 200)) {
    if (s.data !== dataAtual) {
      dataAtual = s.data;
      html += `<div style="font-size:11px;font-weight:700;color:var(--wine);text-transform:uppercase;letter-spacing:0.5px;padding:10px 0 6px">${escHtml(formatarDataBR(s.data))}</div>`;
    }

    const custo = parseFloat(s.custoTotal) || 0;
    const custoTxt = custo > 0
      ? fmtMoeda(custo)
      : '<span style="color:#791F1F" title="Item sem preço de compra registrado">R$ 0,00 ⚠</span>';
    const obs = s.obs ? `<div style="font-size:11px;color:var(--muted);margin-top:2px">${escHtml(s.obs)}</div>` : '';
    ...
    html += obs;
    html += `</div>`;
    html += `<div style="font-size:13px;font-weight:700;color:var(--wine);white-space:nowrap">${custoTxt}</div>`;
    if (podeExcluir) {
      html += `<button class="vinculo-btn-sm danger" data-action="del-saida" data-saida-id="${s.id}">🗑</button>`;
    }
    html += `</div>`;
  }

  if (lista.length > 200) {
    html += `<div style="font-size:11px;color:var(--muted);text-align:center;padding:8px">mostrando os 200 mais recentes de ${lista.length}</div>`;
  }
...
function setupEventos() {
  ...
  document.querySelectorAll('[data-limpeza-subtab]').forEach(b => {
    b.addEventListener('click', () => switchSubTabLimpeza(b.dataset.limpezaSubtab));
  });

  // Limpeza - Saídas (Fase 2)
  $('btn-nova-saida').addEventListener('click', abrirModalSaida);
  $('btn-config-limpeza').addEventListener('click', abrirModalLimpezaConfig);
  $('btn-salvar-saida').addEventListener('click', salvarSaida);
  $('saida-item').addEventListener('change', atualizarPreviewSaida);
  $('saida-qtd').addEventListener('input', atualizarPreviewSaida);
  $('modal-saida').addEventListener('click', e => { if (e.target.id === 'modal-saida') fecharModalSaida(); });
  $('modal-limpeza-config').addEventListener('click', e => { if (e.target.id === 'modal-limpeza-config') fecharModalLimpezaConfig(); });

  $('saidas-filtro-mes').addEventListener('change', e => {
    saidasMes = e.target.value;
    renderSaidas();
  });
  $('saidas-filtro-pessoa').addEventListener('change', e => {
    saidasPessoa = e.target.value;
    renderSaidas();
  });
  $('saidas-lista').addEventListener('click', e => {
    const b = e.target.closest('[data-action="del-saida"]');
    if (b) excluirSaida(b.dataset.saidaId);
  });

  $('btn-add-solicitante').addEventListener('click', adicionarSolicitante);
  $('novo-solicitante-nome').addEventListener('keydown', e => {
    if (e.key === 'Enter') adicionarSolicitante();
  });
  $('limpeza-config-solicitantes').addEventListener('click', e => {
    const b = e.target.closest('[data-action="del-solicitante"]');
    if (b) removerSolicitante(b.dataset.solId);
  });
  ...
  $('search-limpeza-catalogo-clear').addEventListener('click', () => {
    $('search-limpeza-catalogo').value = '';
    searchLimpezaCatalogo = '';
    renderConfigItensLimpeza();
  });
